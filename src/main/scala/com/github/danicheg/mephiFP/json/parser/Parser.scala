...
object Parser {
  ...
  def toToken(input: String): List[Token] = {
    ...
    @tailrec
    def parseStr(rest: List[Char],
                 acc: List[Char] = Nil): (List[Char], List[Char]) =
      rest match {
    ...
    @tailrec
    def loop(rest: List[Char], acc: List[Token] = Nil): List[Token] =
      rest match {
        ...
        case 'n' :: 'u' :: 'l' :: 'l' :: xs =>
          loop(xs, acc ::: List(NullToken))
        case 't' :: 'r' :: 'u' :: 'e' :: xs =>
          loop(xs, acc ::: List(BooleanToken(true)))
        case 'f' :: 'a' :: 'l' :: 's' :: 'e' :: xs =>
          loop(xs, acc ::: List(BooleanToken(false)))
  ...
  def fromToken(tokens: List[Token]): JsonValue = {
    ...
    def parse(tokens: List[Token]): (JsonValue, List[Token]) = {
      ...
      def parseObj(
          rest: List[Token],
          obj: List[(String, JsonValue)] = Nil): (JsonValue, List[Token]) =
        rest match {
      ...
      @tailrec
      def parseArr(rest: List[Token],
                   list: List[JsonValue] = Nil): (JsonValue, List[Token]) =
        rest match {
  ...
  def fromJson(value: JsonValue): String = value match {
    case JsonObject(entries) =>
      val serializedEntries = for ((k, v) <- entries)
        yield s""""$k": ${fromJson(v)}"""
