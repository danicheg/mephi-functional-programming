...
object laboratoryEquations extends App {
  ...
  (equations zip dFunctions zip functionsIterativeMethod).foreach {
    case (((f, bnd, vl), df), fim) =>
      Equations.print(Equations.newtonMethod(f, df, bnd, precision),
                      "Newton Method")
      Equations.print(Equations.dichotomyMethod(f, bnd, epsilon),
                      "Dichotomy Method")
      Equations.print(Equations.iterativeMethod(fim, bnd, precision),
                      "Iterative Method")
  }
...
object TaylorSeries {
  ...
  def getFunctionValues(func: Double => Double,
                        nthElem: (Double, Int) => Double,
                        lb: Double,
                        ub: Double,
                        eps: Double,
                        step: Double) = {
    ...
    def getTaylorSeries = {
      ...
      def isCloseEnough(value: Double, interval: (Double, Double)) =
        value > interval._1 && value < interval._2
      def getElem(point: Double, n: Int) =
        (for (i <- 0 to n) yield nthElem(point, i)).sum
      def getInterval(res: Double) = (res - eps, res + eps)

      @tailrec
      def gen(step: Int,
              from: Double,
              bmk: Double): (Int, Double, Double, Double) =
        if (isCloseEnough(getElem(from, step - 1), getInterval(bmk)))
          (step, from, bmk, getElem(from, step - 1))
        else gen(step + 1, from, bmk)

      def eval(
          sl: List[(Double, Double)]): List[(Int, Double, Double, Double)] =
        sl.map(i => gen(1, i._1, i._2))
  ...
  def printTable(func: Double => Double,
                 nthElem: (Double, Int) => Double,
                 lb: Double,
                 ub: Double,
                 eps: Double,
    ...
    println(
      "number of steps \t\t point \t\t value of standard method \t\t value of Taylor method")
    result foreach {
      case (stp, pnt, vsm, stm) =>
        println(s"$stp \t\t\t\t\t\t $pnt \t\t $vsm \t\t\t $stm")
    }
  }
...
object Equations {
  ...
  def dichotomyMethod(f: Double => Double,
                      bound: (Double, Double),
                      eps: Double) = {
  ...
  def iterativeMethod(f: Double => Double,
                      bound: (Double, Double),
                      eps: Double) = {
